...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    y_ = softmax(np.dot(predicted, outputVectors.T))
    ...
    grad = np.dot(delta.reshape(W, 1), predicted.reshape(1, N))
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    sign = np.array([1.] + [-1.] * K)
    sampleOutputVectors = outputVectors[indices, :]
    sigmoid_values = sigmoid(sign * np.dot(predicted, sampleOutputVectors.T))
    cost = -np.sum(np.log(sigmoid_values))

    weights = sign * (sigmoid_values - 1)
    gradPred = np.dot(weights, sampleOutputVectors)
    grad_diff = np.dot(weights.reshape(K+1, 1), predicted.reshape(1, N))
    for k in xrange(K+1):
        grad[indices[k]] += grad_diff[k,:]
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for contextWord in contextWords:
        ...
        cost_d, gradPred_d, grad_d = word2vecCostAndGradient(
            predicted,
            target,
            outputVectors,
            dataset
        ...
        cost += cost_d
        gradIn[currentWordIndex] += gradPred_d
        gradOut += grad_d
    ### END YOUR CODE
...
def test_word2vec():
    ...
    def print_data(cost, gradIn, gradOut):
        print("cost: %s" % cost)
        print("gradIn: "); print(gradIn)
        print("gradOut: "); print(gradOut)
    ...
    cost, gradIn, gradOut = skipgram("c", 1, ["a", "b"],
        dummy_tokens, dummy_vectors[:5,:], dummy_vectors[5:,:], dataset,
